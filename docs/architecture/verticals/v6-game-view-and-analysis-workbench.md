...
5. System runs the browser-worker engine, persists run and ply snapshots, and updates progress, eval graph, and annotations in place.
...
- Application: route-local orchestration for current-ply state, autoplay, manual-branch state, analysis execution, cancellation, and post-analysis puzzle generation.
...
- Composition: `/game/$gameId` route registration and route-to-engine lifecycle cleanup.
...
- Acceptance criteria:
  ...
  - Starting analysis updates progress in-place and persists the final run snapshot.
...
- The `/game/$gameId` route still owns significant application orchestration and needs further extraction.
