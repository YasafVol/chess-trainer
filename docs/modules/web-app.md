...
- The root signed-out shell now builds route-aware auth-gate copy from presentation helpers so public deep links like `/library`, `/puzzles`, `/backoffice`, and `/game/$gameId` preserve context before Google sign-in.
...
- Engine flavor selection is now centralized so current prod/preview builds bundle only `stockfish-18-lite-single`, while heavier `stockfish-18-single` and `stockfish-18` flavors remain prepared in source for a later dedicated asset rollout.
