...
let cancelled = false;
let currentFlavor: EngineInitMessage["payload"]["flavor"] | null = null;
...
function parseUciList(raw: string | undefined): string[] | undefined {
  if (!raw) {
    return undefined;
  }
  const moves = raw
...
function parseBestmoveLine(line: string): string | undefined {
...
function waitForLine(match: (line: string) => boolean, timeoutMs: number = 10_000): Promise<string> {
...
function parseInfoLine(line: string): {
  ...
  const evaluationType = (scoreMatch?.[1] as "cp" | "mate") ?? "cp";
...
function handleEngineLine(line: string) {
  ...
  if (line.startsWith("bestmove")) {
    ...
    const outbound: WorkerOutbound = activeAnalyze.cancelled
...
async function ensureEngine(flavor: EngineInitMessage["payload"]["flavor"]) {
  ...
  engine.onerror = (event: Event | ErrorEvent) => {
...
self.onmessage = async (event: MessageEvent<WorkerInbound>) => {
  ...
  if (message.type === "engine:analyzePosition") {
    ...
    try {
      ...
      const completion = new Promise<WorkerOutbound>((resolve, reject) => {
        activeAnalyze = {
          requestId: message.requestId,
