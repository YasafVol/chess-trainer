...
test("buildAnalysisPlan analyzes every ply up to 200 plies at default depth", () => {
  ...
  assert.deepEqual(
    plan.map((step: { ply: number }) => step.ply),
    [0, 1, 2, 3, 4]
  );
  assert.ok(plan.every((step: { depth: number }) => step.depth === 16));
});
...
test("buildAnalysisPlan samples every second ply for games between 201 and 300 plies", () => {
  ...
  assert.ok(plan.every((step: { depth: number }) => step.depth === 14));
  assert.ok(plan.every((step: { ply: number }) => step.ply % 2 === 0 || step.ply === totalPlies));
});
...
test("buildAnalysisPlan samples every fourth ply but keeps tactical and final positions for long games", () => {
  ...
  const plies = plan.map((step: { ply: number }) => step.ply);

  assert.ok(plies.includes(7), "tactical ply must be included");
  assert.ok(plies.includes(totalPlies), "final ply must be included");
  assert.ok(
    plan.every((step: { depth: number }) => step.depth === 12),
    "all sampled plies in long games should use depth 12"
  );
  assert.ok(
    plan.every((step: { ply: number }) => step.ply % 4 === 0 || step.ply === 7 || step.ply === totalPlies),
    "non-tactical long-game plies should follow every-fourth sampling"
  );
});
